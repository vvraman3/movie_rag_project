...
def get_filter(query):
    # Prompting for exact match to the 'movie_name' metadata key
...
if query:
    ...
    if results['documents']:
        ...
        for score, meta in final:
            with st.container():
                st.write(f"### {meta['movie_name'].upper()}")
                # Note: For local video playback, ensure the path is correct for your OS
